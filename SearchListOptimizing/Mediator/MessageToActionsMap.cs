...
namespace SearchListOptimizing.Mediator
{
    ...
    internal class MessageToActionsMap
    {
        ...
        internal void RemoveActionsForTarget(object target)
        {
            if (target == null)
            ...
            lock (map)
            {
                var emptyMessageList = new List<string>();

                foreach (var kvp in map)
                {
                    kvp.Value.RemoveAll(x => x.Target == target);

                    if(kvp.Value.Count == 0)
                    {
                        emptyMessageList.Add(kvp.Key);
                    }
                }

                foreach (var message in emptyMessageList)
                {
                    if(map.ContainsKey(message))
                    {
                        map.Remove(message);
